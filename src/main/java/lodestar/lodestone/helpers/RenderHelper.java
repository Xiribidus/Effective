package lodestar.lodestone.helpers;
...
//	public static ShaderEffect getShader(RenderLayer type) {
//		if (type instanceof RenderLayer.CompositeRenderLayer compositeRenderLayer) {
//			Optional<Supplier<ShaderInstance>> shader = compositeRenderLayer.state.shaderState.shader;
//			if (shader.isPresent()) {
...
//	}
	...
	public static void vertexPosUVLight(VertexConsumer vertexConsumer, Matrix4f last, float x, float y, float z, float u, float v, int light) {
		vertexConsumer.vertex(last, x, y, z).uv(u, v).overlay(light).next();
	}
	...
	public static void vertexPosColorUVLight(VertexConsumer vertexConsumer, Matrix4f last, float x, float y, float z, float r, float g, float b, float a, float u, float v, int light) {
		vertexConsumer.vertex(last, x, y, z).color(r, g, b, a).uv(u, v).overlay(light).next();
	}
